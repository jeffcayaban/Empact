...
import NoCreatorArgumentAlert from "../../../common/alerts/NoCreatorArgumentAlert";
...
/**
 * Will be displayed when the user decides to create an argument that disagrees with a petition. It will display
 * relevant critical questions that the user can choose to answer.
 */

class PetitionDisagree extends React.Component {
    ...
    render() {
        ...
        const { petition, isPetitionClosed, isLoading } = this.state;
        ...
        if (!isLoading) {
            ...
            if (petition !== null && petition !== undefined) {
                ...
                if (username && (petitionCreator.username !== username) && !isPetitionClosed) {
                    // Assumes the user is logged in, and is not the petition creator and that the petition is not closed.

                    return (
                        <div className={"secondaryPageContainerStyle"}>
                            <div className={"centerStyle"}>
                                <h2 align="center">You <strong>disagree</strong> with the petition</h2>
                                <p align="center">Tell us your reasons for deciding this!</p>
                            </div>

                            <div style={{ paddingTop: '30px' }}>
                                <SituationCQ1 petitionId={petitionId} isSupporting={false} situation={petition.situation} />
                                <ActionCQ3 petitionId={petitionId} isSupporting={false} petition={petition} />
                            </div>
                            <Route render={({ history }) => (
                                <Button onClick={() => { history.push(`/petition/${petitionId}`); }}>
                                    Back
                                </Button>
                            )}/>
                        </div>
                    );
                } else if (isPetitionClosed && username) {
                    // Assumes the petition is closed
                    return <NoArgCreationClosedPetition />
                } else if (username && (petitionCreator.username === username)) {
                    // Will display an alert when the user is the petition creator.
                    return <NoCreatorArgumentAlert />
                } else {
...
export default PetitionDisagree;